...
beforeAll(async () => {
	...
	await conn.execute("CREATE TABLE rel_book_tag (book_id TEXT, tag_id TEXT)");
	await conn.execute(
		"INSERT INTO rel_authors VALUES ('a1', 'Ada'), ('a2', 'Linus')",
	);
	await conn.execute(
		"INSERT INTO rel_books VALUES ('b1', 'a1', 'Engines'), ('b2', 'a1', 'Looms'), ('b3', 'a2', 'Kernels')",
	);
	await conn.execute(
		"INSERT INTO rel_tags VALUES ('t1', 'classic'), ('t2', 'tech')",
	);
	await conn.execute(
...
describe("atlas > relations against real SQLite", () => {
	...
	it("preloads a belongsTo relation", async () => {
		const books = await new BaseRepository(Book, conn)
			.query()
			.preload("author")
			.exec();
	...
	it("preloads a manyToMany relation through the pivot", async () => {
		...
		expect(engines?.tags.map((t) => t.label).sort()).toEqual([
			"classic",
			"tech",
		]);
