...
import { BaseEntity, BaseRepository, Column, Entity, PrimaryKey } from "../../src/index.js";
...
function sql(build: (query: ReturnType<typeof q>) => unknown, dialect?: "sqlite" | "postgres") {
...
describe("atlas > ModelQuery builder → SQL", () => {
	...
	it("whereBetween / whereNotBetween", () => {
		...
		expect(sql((b) => b.whereNotBetween("age", [1, 5]))).toMatch(/NOT BETWEEN/i);
	});
	...
	it("a where callback produces a parenthesised group", () => {
		const out = sql((b) => b.where((g) => g.where("status", "a").orWhere("status", "b")));
		expect(out).toMatch(/\(/);
	});
