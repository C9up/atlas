...
export class ModelQuery<T extends BaseEntity> {
	...
	async #resolveThrough(
		...
		// Map secondLocal (= through PK by default) → parentId. When the caller
		// overrides `secondLocalKey` to a non-unique column, two intermediate
		// rows can share the same key and the second would silently overwrite
		// the first — so we detect duplicates and throw a clear error instead
		...
		for (const row of throughRows) {
			...
			if (
				...
				throw new Error(
					`@HasManyThrough/@HasOneThrough '${relationName}': duplicate secondLocalKey='${String(key)}' ` +
						`on ${throughTable} maps to multiple parents. Either set secondLocalKey to a unique column ` +
						`(default: ${throughClass.name}.${throughPk}) or fix the underlying data.`,
				);
			}
